...
2.  first class function. In JavaScript, functions are treated like values (citizens). This is what
    makes them “first-class.” - Assign a function to a variable - Pass a function as an argument to
    another function - Return a function from another function Example:
    ...
    Because functions are first-class, we can build callbacks, higher-order functions, promises,
    async/await, etc.

3.  call back function.

- A callback is a function that is passed as an argument to another function and is executed later
  (when the parent function decides).

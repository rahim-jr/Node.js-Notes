...
5. IIFE.
    ...
    - Used to avoid polluting global scope. Example:
...
6. lexical environment.
    - The environment a function has access to, based on where it was defined (not where it’s
      called). It contains:
    - Local variables
...
7. scope chain.
    ...
    - JS looks for a variable in the current scope → if not found, it moves outward to the parent,
      then global.
...
Defines where a variable is accessible. Types in JS: - Global Scope → accessible everywhere -
Function Scope → accessible only inside that function - Block Scope (let, const) → accessible only
inside {}

1.  variable name being saved as memory cell name.
    ...
    - The value can change, but the label (x) stays attached to the memory cell until the scope
      ends.

2.  execution context.
    - Execution Context = the environment in which a piece of JS code runs.
    - It contains all the information the engine needs to execute code: variables, functions, and
      this. Types:
...
6.  execution phase.
    ...
    - In this phase: - Variable assignments happen (name = "Rahim") - Functions get executed when
      called. Using the example above: - sayHi() → outputs "Hello" - name = "Rahim"
...
7.  callstack.
    ...
    Global Context → one() → two() → console.log()

    After two() finishes → popped After one() finishes → popped Back to global
